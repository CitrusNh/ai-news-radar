...
def test_workflow_ingests_successful_sources_and_isolates_failures():
    store = InMemoryStore()
    store.sources = {
        "ok": Source("ok", "成功来源", feed_url="https://example.com/ok", robots_status="allowed", default_channel="模型与产品"),
        "bad": Source("bad", "失败来源", feed_url="https://example.com/bad", robots_status="allowed"),
        "blocked": Source("blocked", "禁止来源", feed_url="https://example.com/blocked", robots_status="blocked"),
    }
...
def test_workflow_is_idempotent_for_existing_urls():
    store = InMemoryStore()
    source = Source("ok", "来源", feed_url="https://example.com/feed", robots_status="allowed", default_channel="模型与产品")
